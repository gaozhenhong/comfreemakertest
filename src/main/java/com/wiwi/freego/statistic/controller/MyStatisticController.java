...
public class MyStatisticController extends BaseController
{
  ...
  public String myStatisticList(Model model) throws Exception {
    ...
    long orderCount = 0L;
    JSONArray jsonArray = this.service.getStatisticList(companyId);
    ...
    model.addAttribute("settlementStatistic", this.service.getSettlementStatistic(companyId));
    model.addAttribute("JSONUtil", JSONUtil.class);
    model.addAttribute("Freego", HotelOrder.ORDER_ORIGIN_FREEGO);
    return "/statistic/myStatistic/myStatisticList";
  }
}